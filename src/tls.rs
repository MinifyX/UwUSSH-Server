...
pub fn fingerprint_of(data_dir: &Path) -> io::Result<String> {
    Ok(load_or_create(data_dir, &["localhost".to_string()])?.fingerprint)
}
...
/// Write a private key so that only its owner can read it. On Windows this is
/// a plain write — that side runs from a developer's own folder, while the
/// servers that matter are the ones in a container.
